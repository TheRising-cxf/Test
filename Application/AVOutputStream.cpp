...
bool CAVOutputStream::OpenOutputStream(const char* out_path)
{
    ...
    if(m_video_codec_id != 0)
    {
		...
		m_out_buffer = (uint8_t *)av_malloc(avpicture_get_size(AV_PIX_FMT_RGB32, pCodecCtx->width, pCodecCtx->height));
		avpicture_fill((AVPicture *)pFrameRGB, m_out_buffer, AV_PIX_FMT_RGB32, pCodecCtx->width, pCodecCtx->height);
    }
