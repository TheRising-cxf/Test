...
bool CAVOutputStream::OpenOutputStream(const char* out_path)
{
	...
	m_show_buffer = (uint8_t *)av_malloc(avpicture_get_size(AV_PIX_FMT_RGB32, m_width, m_height));
	avpicture_fill((AVPicture *)pFrameRGB, m_show_buffer, AV_PIX_FMT_RGB32, m_width, m_height);
    return true;
}
int CAVOutputStream::show_video_frame(AVStream * input_st, enum AVPixelFormat pix_fmt, AVFrame *pframe, AVCodecContext* ctx)
{
	...
	if (img_convert_ctx_rgb == NULL)
	{
		...
		//param4: dst_width
		//param5: dst_height
		img_convert_ctx_rgb = sws_getContext(m_width, m_height,
			pix_fmt, ctx->width, ctx->height, AV_PIX_FMT_RGB32, SWS_BICUBIC, NULL, NULL, NULL);
	}

	sws_scale(img_convert_ctx_rgb, (const uint8_t* const*)pframe->data, pframe->linesize, 0, ctx->height, pFrameRGB->data, pFrameRGB->linesize);
	return 0;
}
//input_st -- 输入流的信息
...
int CAVOutputStream::write_video_frame(AVStream * input_st, enum AVPixelFormat pix_fmt, AVFrame *pframe, int64_t lTimeStamp)
{
   ...
   if(m_first_vid_time1 == -1)
   {
    ...
    if(img_convert_ctx == NULL)
    {
		...
		img_convert_ctx_rgb = sws_getContext(m_width, m_height,
			pix_fmt, pCodecCtx->width, pCodecCtx->height, AV_PIX_FMT_RGB32, SWS_BICUBIC, NULL, NULL, NULL);
    }
	...
	sws_scale(img_convert_ctx_rgb, (const uint8_t* const*)pframe->data, pframe->linesize, 0, pCodecCtx->height, pFrameRGB->data, pFrameRGB->linesize);
#endif
